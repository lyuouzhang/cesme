...
##cesme without SCAD
##only for low dimension
cesme_lowd = function(y, K, thre=NULL, N=100, C0=1, C2=1, kap=0.8, z.initial=NULL, mu.initial=NULL,
                 maxit_msda=as.integer(1e+3), eps_msda=1e-4, ncvx=FALSE,Crho = 0,
  ...
  ## check if data is low dimensional
  if(p>=n.smp){
    stop("Dimension larger than sample size. Use cesme for high dimensional clustering.")
  }
  ...
  for (iter in seq(N - 1L)) {
    ...
    for(i in seq(S)){
      ...
      delta = t(crossprod(rbind(1,diag(-1,K-1)),mu.iter[,,i]))
      ...
      gamma2.iter[,,(i-1)*L+1] = solve(sigma,delta)
      tt1 = Sys.time()
          
      logw.tmp = alpha.fn(g.tmp=g.iter[,,1], mu.tmp=mu.iter[,,i],
                              pro.tmp=pro.iter[i,], gamma.tmp=t(gamma2.iter[,,(i-1)*L+1]),
                              n=n.smp, p=p, K=K)
      w.tmp <- exp(-logw.tmp)
      w.tmp[is.na(w.tmp)] <- 0
      w.tmp[is.infinite(w.tmp)] <- 10^6
      alpha.temp[1,,] = w.tmp/rowSums(w.tmp)
      pro.tmp = colMeans(alpha.temp[1,,])
      z2.iter[i,,1] = z.fn(g.tmp=g.iter[,,1], mu.tmp=mu.iter[,,i],
                               pro.tmp=pro.tmp, gamma.tmp=t(gamma2.iter[,,(i-1)*L+1]),
                               n=n.smp, p=p, K=K)
          
      if((length(unique(z2.iter[i,,1])) < K) || (min(table(z2.iter[i,,1])) < 2L)){
        z2.iter[i,,1] = z.iter[iter,,1]
      }else{
        tt2 = Sys.time()
        ...
        bic3.iter[i,1] = -2 * llh + log(max(n.smp,p)) * (2*K*p+p^2)
      }
      ...
      pro.iter[i+1,] = colMeans(alpha.temp[1,,])
      ...
      mu.iter[,,i+1] = crossprod(alpha.temp[1,,], 
                                 g.iter[,,1]) / colSums(alpha.temp[1,,])
      ...
      if(p<=5000){
        sigma.iter[,,i+1] = 
          sigma.est.fn(mu=mu.iter[seq(K),,i+1], 
                       g=g.iter[,,1], alpha=alpha.temp[1,,seq(K)], 
                       n=n.smp, p=p, K=K, n.inv=n.inv)
      }else{
        sigma.iter[[i+1]] = 
      ...
      ############################################################################
      ## BIC
      bic1.iter[iter,i] = bic3.iter[i,1]
